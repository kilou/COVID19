...
pred.covid <- function(
  ...
  mlos <- pars$mlos[ind]
  vlos <- pars$vlos[ind]
  if(method=="pac"){pac <- pars$pac[ind]}
  ...
  if(method%in%c("pic_at_hos_entry","pac")){
    # Calculate incident cases that will require IC at some point
    for(k in 1:(j+nday)){ninc[,k] <- round(rpic(nsim,mpic[k],vpic[k])*ninc[,k])}
  }
  
  # Calculate nb of ICU beds required (function to be parallelized)
  fun <- function(s){
    npat <- ninc[s,] # nb of patient hospitalized on day j that will require IC at some point
    ...
    if(method=="pic_at_hos_entry"){
      icu.out <- icu.in+los-1
    }
    
    if(method=="pic_at_icu_entry"){
      icu.out <- icu.in+los-1
      for(k in 1:(j+nday)){
        sel <- which(icu.in==k); nsel <- length(sel)
        if(nsel>0){
          pic <- rbinom(nsel,size=1,prob=rpic(1,mpic[k],vpic[k]))
          icu.in[sel] <- icu.in[sel]*pic
          icu.out[sel] <- icu.out[sel]*pic
        }
      }
    }
