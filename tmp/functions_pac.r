...
pred.covid <- function(
  ...
  fun <- function(s){
    ...
    # Select ICU patients that will be really admitted in ICU based on pac on the day of admission
    for(k in 1:(j+nday)){
      sel <- which(day.in==k); nsel <- length(sel)
      if(nsel>0){
        adm <- rbinom(nsel,size=1,prob=pac[k]) # binary indicator for admission in ICU
