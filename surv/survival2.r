...
# MODEL FOR ICU LAG
# one <- lag/lag
# fm10 <- survreg(Surv(lag,one)~cut(age,breaks=c(0,64,74,104),include.lowest=T),dist="weibull"); aic(fm10)
# summary(fm10)
# 
# #****************************************
# formula.lag <- Surv(lag,rep(1,length(lag)))~cut(age,breaks=c(0,64,74,104),include.lowest=T)
# #****************************************
# col.lag <- c("green","red")
# mid.lag <- tapply(age,cut(age,breaks=c(0,64,74,104),include.lowest=T),mean)
# 
# # Fit model for lag
